...
class OrderController extends MyController{
	...
	public function done(){
		...
		$v_url = 'http://www.zhangmenren.com/index.php/Home/Order/access';//告诉第三方支付平台，把支付结果返回到该页面
		$v_oid = $order_sn;//订单编号
		$v_amount = $order_info['goods_amount'];//订单金额
		$key = '#(%#WU)(UFGDKJGNDFG';
	...
	public function access(){
		...
		if($v_md5str==$md5info){
			...
			if($v_pstatus==20){
				//支付成功后，要修改支付状态
				$where['order_sn'] = $v_oid;
				$model = M(OrderInfo);
				$rs = $model->where($where)->setField('pay_status',1);
				if($rs !== false){
					$this->success('恭喜你支付成功',U('Index/index'));
					exit;
				}
			}else{
				$this->error('不要意思，支付失败，请重新支付');
			}
		}else{
			$this->error('数据有诈');
		}
